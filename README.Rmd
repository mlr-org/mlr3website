...
1. Install the `mlr3website` package with `R CMD INSTALL .`.
...
The workflow uses the dependencies from the `mlr3website` package.
This e.g. ensures that the overview lists are up to date.

**gallery-weekly.yml**

The gallery is rendered in a Docker container (`mlrorg/mlr3-gallery`) which includes all required packages.
Runs once a week.
