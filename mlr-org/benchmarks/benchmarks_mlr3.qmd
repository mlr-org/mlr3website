...
create_table_task_evals = function(data, task = "data_1000") {
  ...
  setcolorder(data, c("mlr3", "paradox", "evals", "median_runtime", "k_1000", "k_100", "k_10", "k_1", "median_memory", "pk_1000", "pk_100", "pk_10", "pk_1"))
...
The runtime of the `resample()` function with `future::multisession` parallelization is also measured for different mlr3 versions.
...
#| column: body-outset
#| tbl-cap: |
#|   Runtime and memory usage of resample() by mlr3 version and resampling iterations.
#|   The k factors indicate how many times longer the total runtime is compared to the model training time.
#|   The numbers represent the model training time itself e.g. k100 are models trained for 100 ms.
#|   A green background highlights cases where the total runtime is less than three times the model training time.
#|   The pk factors indicate how many times faster the parallel runtime is compared to the sequential runtime.
#|   The numbers are NA if the parallel runtime is slower than the sequential runtime.
...
The resampling iterations (`evals`) are set to 1000, 100, and 10.
The runtime of the `benchmark()` function with `future::multisession` parallelization is also measured for different mlr3 versions.
The parallelization is conducted on 10 cores.
...
#| column: body-outset
#| tbl-cap: |
#|   Runtime and memory usage of benchmark() by mlr3 version and dataset size.
#|   The k factors indicate how many times longer the total runtime is compared to the model training time.
#|   The numbers represent the model training time itself e.g. k100 are models trained for 100 ms.
#|   A green background highlights cases where the total runtime is less than three times the model training time.
#|   The pk factors indicate how many times faster the parallel runtime is compared to the sequential runtime.
...
#|  The size is measured in memory and after calling `serialize()` and `unserialize()`.
create_table_object_size_task_evals(results_object_size$resampling_instantiate, task = "data_1000")
```
...
#|  The size is measured in memory and after calling `serialize()` and `unserialize()`.
create_table_object_size_task_evals(results_object_size$benchmark_result, task = "data_1000")
```
...
#|  The size is measured in memory and after calling `serialize()` and `unserialize()`.
create_table_object_size_task_evals(results_object_size$benchmark_result_store_models, task = "data_1000")
```

# Version History

This section provides an overview of the latest changes in the mlr3 and paradox packages regarding memory usage and runtime.
