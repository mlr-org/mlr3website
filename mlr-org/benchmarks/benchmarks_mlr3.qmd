...
con = dbConnect(RSQLite::SQLite(), here::here("mlr-org/benchmarks/benchmark_results.db"))
...
The mlr3 package comes with the `resample()` and `benchmark()` functions to evaluate models.
This report compares the runtime and memory usage of these functions across different versions of mlr3
Given the extensive package ecosystem of mlr3, performance bottlenecks can arise at various stages.
This report aims to help you determine whether the runtime of your workflow falls within expected ranges.
If you observe significant runtime or memory anomalies, please report them by opening a GitHub issue.
...
Models with training times of 100 ms and 1 second exhibit minimal overhead when using `resample()` and `benchmark()`.
For models with a training time of 10 ms, the overhead approximately doubles the runtime.
When the training time is 1 ms, the runtime becomes ten times larger than the model training time.
...
The encapsulation of the `$train()` method introduces a runtime overhead of 3 seconds per model training session when using the `callr` package.
