...
First, we start by imputing missing values in the [diabetes data set](https://mlr3.mlr-org.com/reference/mlr_tasks_diabetes.html).
...
In this example, we use the [diabetes data set](https://mlr3.mlr-org.com/reference/mlr_tasks_diabetes.html) which is used to predict whether or not a patient has diabetes.
...
```{r index-013, fig.width=10, fig.height=10}
graph = po("encode") %>>%
  po("imputeoor") %>>%
  learner
