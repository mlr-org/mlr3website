...
After drawing a population of configurations, Hyperband iteratively allocates more resources to promising configurations and terminates low-performing ones.
This type of optimization is called *multi-fidelity* optimization.
The fidelity parameter is part of the search space and influences the computational cost of fitting a learner.
...
Hyperband is going to control the fidelity with the `frac` parameter.
For this, we have to connect the `r ref("PipeOp")` with the learner.
...
The graph learner subsamples before passing the data to the learner.
The parameter set of the graph learner is a combination of the `r ref("PipeOp")` and learner set.
...
The fidelity parameter is tagged with `"budget"` in the search space.
...
The parameter `eta` controls how many configurations are terminated in each iteration.
...
```{r}
hyperband_schedule(r_min = 2^-3, r_max = 1, eta = 2)
```
...
The data set size is increased in four steps from 12.5% to 100%
