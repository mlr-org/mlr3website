...
It implements the `r ref("mlr_fselectors_rfe", "RFE")` algorithm.
...
We load the callback with the `r ref("clbk()")` function and pass it to the `callback` argument of the `r ref("fsi()")` function.
