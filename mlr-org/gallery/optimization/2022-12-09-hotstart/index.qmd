...
For example, training an XGBoost model for additional 500 boosting iterations.
Hotstarting in mlr3 works with a cache of already trained models.
This cache is called a `r ref("HotstartStack")` which is automatically created while tuning.
...
ggplot(data_3, aes(x = runtime, y = batch_nr, group = id)) +
  ...
  scale_colour_discrete(name = "hotstarting", labels = c("Activated", "Deactivated")) +
  ylab("Batch") +
...
The parallelization package future copies the models sequentially to the workers.
...
We might fix forward hotstarting in the future with workers that read hot start models from a database in parallel.
However, the need for this seems to be rather small.
Models that can be trained iteratively usually have a very efficient internal early-stopping mechanism.
...
Training the model from the beginning would have taken about one minute.
Without parallelization, we get the expected speed advantage.

# Conclusion

Backward hotstarting significantly reduces the time to tune a random forest.
Whereas forward hotstarting is less useful due to the parallelization overhead and the very efficient internal early stopping mechanisms.
