---
title: "Shadow Variable Search on the Diabetes Data Set"
description: |
...
As an example, we will search for the optimal set of features for a `r ref("mlr_learners_classif.svm", "support vector machine")` on the `r ref("mlr_tasks_diabetes", text = "diabetes")` data set.
...
The objective of the `r ref("mlr_tasks_diabetes", text = "diabetes")` data set is to predict whether a person has diabetes or not.
The data set includes 128 patients with 8 measurements (see @fig-features).
...
#| fig-height: 3
#| label: fig-features
#| fig-cap: "Distribution of the features in the diabetes data set."
#| warning: false
...
The feature glucose was selected in the first iteration.
In the second iteration, a shadow variable achieved the best performance and the feature selection was terminated.
