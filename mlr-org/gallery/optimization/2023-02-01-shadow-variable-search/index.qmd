...
`r ref_pkg("mlr3fselect")` is the feature selection package for the [mlr3 ecosystem](https://mlr-org.com/ecosystem.html).
...
Now we construct the feature selection problem by using the `r ref("fsi()")` function to construct an `r ref("FSelectInstanceSingleCrit")`.
...
The feature age was selected first and in the following iterations glucose, insulin and mass.
...
data = as.data.table(instance$archive)[, head(.SD, 1), by = batch_nr]
