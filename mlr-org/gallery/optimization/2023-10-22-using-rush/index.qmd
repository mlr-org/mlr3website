...
The default of `$fetch_finished_tasks()` is `c("xs", "xs_extra", "worker_extra", "ys", "ys_extra", "state")`.
...
The methods `$fetch_results()` and `$fetch_finished_tasks()` cache the already queried data (see @sec-caching).

The `data_format = "list` returns a `list` instead of a `data.table`.

```{r}
rush$fetch_finished_tasks(data_format = "list")
```

An advantage of the `data.table` format is that we can set a key on the result column.
The `data.table` is then always sorted by the result which gives us fast access to the best result.
However, when we want to store complex data structures like `list`, `data.frame` and `environment` in the database, we need to wrap them in a `list`.
Otherwise, `data.table` cannot bind the tasks to a table.
In this case, it might be better to use the `list` format to avoid wrapping the objects in lists.

## Wait for Results {#sec-wait-for-results}

The `$wait_for_*()` variants of the `$fetch_*` methods wait until a new result is available.

```{r}
rush$wait_for_latest_results()
```
...
We can set a timeout with the `timeout` argument.
...
The cache is a `data.table` or `list` that is stored in the controller.
...
Rush uses a random number generator with multiple independent streams created by @Ecuyer2002.
The implementation of the `r ref_pkg("rlecuyer")` package is used.
We attach a random number stream to each task.
This ensures reproducibility regardless of the number of workers.
Set a seed when starting the workers to get reproducible results.
...
```{r}
#| echo: false
#| label: fig-sleep-benchmark-1
#| fig-cap: "Runtime of a random search in seconds on 100 hyperparameter configuration with 3-fold cross-validation on 3 workers depending on batch size and chunk size for different training times. The first three runs use a constant training time of 10 ms, 100 ms and 1 second. The last three runs use a heterogenous training time ranging from 10 ms to 10 seconds. Rush does not have a batch size and chunk size parameter."
library(data.table)
library(mlr3misc)
library(ggplot2)
library(patchwork)
...
```{r}
#| echo: false
#| label: fig-sleep-benchmark-2
#| fig-cap: "Runtime of a random search in seconds on 1000 hyperparameter configuration with 3-fold cross-validation on 30 workers depending on batch size and chunk size for different training times. The first three runs use a constant training time of 10 ms, 100 ms and 1 second. The last three runs use a heterogenous training time ranging from 10 ms to 10 seconds. Rush does not have a batch size and chunk size parameter."
