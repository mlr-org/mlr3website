...
In this example, we use the [diabetes data set](https://mlr3.mlr-org.com/reference/mlr_tasks_diabetes.html) which is used to to predict whether or not a patient has diabetes.
...
```{r index-006, fig.width=10, fig.height=10}
graph =
...
```{r index-009, fig.width=10, fig.height=10}
...
```{r index-015, fig.width=12, fig.height=4}
#| column: page
autoplot(instance, type = "marginal",
  cols_x = c("kknn.k", "svm.cost", "ranger.mtry"))
```
