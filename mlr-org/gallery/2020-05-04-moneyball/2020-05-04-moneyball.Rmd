...
table_responsive(head(as.data.table(oml_data)))
```
...
table_responsive(t(task$missings()))
```
...
table_responsive(as.data.table(mlr_measures))
```
...
table_responsive(rr$score(msrs(c("regr.mae", "regr.mse")))[, .(iteration, task_id, learner_id, resampling_id, regr.mae, regr.mse)])
```
...
table_responsive(data.table(t(sort(graph_learner$model$regr.ranger$model$variable.importance, decreasing = TRUE))))
```
