...
```{r index-008, fig.width=10, fig.height=10}
...
We will use the `r ref("mlr_terminators_none", "TerminatorNone")` (i.e., no early termination) for terminating the tuning because we will apply a `r ref("TunerBatchGridSearch", "grid search")` (we use a `grid search` because it gives nicely plottable and understandable results but if there were much more parameters, `r ref("TunerBatchRandomSearch", "random search")` or more intelligent optimization methods would be preferred to a `grid search`:
...
```{r index-019, graph2, fig.width=10, fig.height=8}
...
```{r index-023}
tune_ps2$extra_trafo = function(x, param_set) {
...
```{r index-025}
tune_ps2$extra_trafo(x, tune_ps2)
```
