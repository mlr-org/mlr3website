...
```{r index-017, echo = FALSE, out.width = "100%"}
...
```{r index-019, eval = FALSE, tidy = FALSE}
...
```{r index-020, eval = FALSE, tidy = FALSE}
    .get_state = function(task) {
...
```{r index-021, eval = FALSE, tidy = FALSE}
    .transform = function(task, phase) {
      target = task$data(cols = task$target_names)[[1L]]
...
```{r index-022, eval = FALSE, tidy = FALSE}
    .train_invert = function(task) {
...
```{r index-023, eval = FALSE, tidy = FALSE}
    .invert = function(prediction, predict_phase_state) {
...
```{r index-026, fig.width=10, fig.height=10}
