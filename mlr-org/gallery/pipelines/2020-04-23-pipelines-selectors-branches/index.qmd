...
The `r ref("mlr_tasks_diabetes", "diabetes classification task")` will be used.
...
* `r ref("PipeOpImputeHist")` on the features "glucose", "mass" and "pressure" and
* `r ref("PipeOpImputeMedian")` on the features "insulin" and "triceps".
...
```{r index-007, fig.height = 10, fig.width = 10}
...
```{r index-009, fig.height = 10, fig.width = 10}
...
```{r index-011, fig.height = 6, fig.width = 6}
...
```{r index-014, fig.height = 6, fig.width = 6}
...
```{r index-017, fig.height = 7, fig.width = 7}
...
```{r index-018, fig.height = 7, fig.width = 7}
graph = graph_branch %>>%
  lrn("classif.rpart")
