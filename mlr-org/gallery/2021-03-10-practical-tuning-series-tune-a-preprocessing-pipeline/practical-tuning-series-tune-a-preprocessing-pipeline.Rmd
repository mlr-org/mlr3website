...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-014, fig.width=10, fig.height=10}
graph = po("encode") %>>%
  po("imputeoor") %>>%
  learner
...
saveRDS(bmr, here::here("data/bmr_1.rda"))
```

```{r practical-tuning-series-tune-a-preprocessing-pipeline-026, echo = FALSE}
bmr = readRDS(here::here("data/bmr_1.rda"))
```
...
```{r practical-tuning-series-tune-a-preprocessing-pipeline-029, eval = FALSE}
at_2$train(task)
```
...
#| include: false
utils::download.file(
  url = "https://mlr-org.com/gallery/2021-03-10-practical-tuning-series-tune-a-preprocessing-pipeline/practical-tuning-series-tune-a-preprocessing-pipeline_files/figure-html5/practical-tuning-series-tune-a-preprocessing-pipeline-027-1.png",
