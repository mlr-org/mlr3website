...
In the following example, a custom resampling is created with repeated k-fold CV indices:

```{r index-022}
task_gc = tsk("german_credit")
# Create repeated k-fold CV indices (replaces caret::createMultiFolds)
create_multi_folds = function(n, k = 5, times = 10) {
  result = vector("list", k * times)
  nms = character(k * times)
  idx = 0L
  for (t in seq_len(times)) {
    folds = sample(rep_len(seq_len(k), n))
    for (fold in seq_len(k)) {
      idx = idx + 1L
      result[[idx]] = sort(which(folds != fold))
      nms[idx] = paste0("Fold", fold, ".Rep", t)
    }
  }
  setNames(result, nms)
}
train_ind = create_multi_folds(task_gc$nrow, k = 5, times = 10)
