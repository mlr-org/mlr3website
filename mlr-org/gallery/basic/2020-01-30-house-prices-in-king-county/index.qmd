...
```{r index-021, fig.width=10, fig.height=8}
plot(lrn$model)
text(lrn$model)
```
...
::: {.callout-warning}
The `stamen_watercolor` base map is served by Stadia Maps, which took over the Stamen tile sets in 2023 and requires a free API key.
Register a key with `ggmap::register_stadiamaps()` before running the chunk below, otherwise the tile request fails.
:::

```{r index-022, message = FALSE, eval = FALSE}
