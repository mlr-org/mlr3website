...
We want to find and remove correlated variables given a `cutoff` parameter that specifies the maximum correlation allowed between variables.
...
search_space = ps(
  ...
  .extra_trafo = function(x, param_set) {
    ...
    x$select.selector = function(task) {
      ...
      # find correlated features to drop (replaces caret::findCorrelation)
      abs_cor = abs(cor(data))
      diag(abs_cor) = 0
      drop = character(0)
      nms = colnames(abs_cor)
      while (nrow(abs_cor) > 1L) {
        max_cor = max(abs_cor[upper.tri(abs_cor)])
        if (max_cor <= cutoff) break
        idx = which(abs_cor == max_cor, arr.ind = TRUE)[1L, ]
        avg = colMeans(abs_cor)
        drop_i = idx[which.max(avg[idx])]
        drop = c(drop, nms[drop_i])
        abs_cor = abs_cor[-drop_i, -drop_i, drop = FALSE]
        nms = nms[-drop_i]
      }
      setdiff(fn, drop)
    }
...
```{r index-005}
# trafo is now defined in ps() above
```
