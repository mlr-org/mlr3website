...
First, we take an example task with missing values (`r ref("mlr_tasks_diabetes", "diabetes")`) and create the `r ref("mlr_learners_classif.ranger", "ranger learner")`:
...
```{r index-007, message = FALSE, warning = FALSE, fig.width=10, fig.height=8}
...
```{r index-015, fig.width=6, fig.height=8}
graph$plot()
```
...
As the (`r ref("mlr_tasks_diabetes", "diabetes")`) task does not have any factor levels, we use the
