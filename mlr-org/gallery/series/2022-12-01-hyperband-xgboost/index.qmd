...
This means Hyperband will allocate more boosting iterations to well-performing configuration.
The number of boosting iterations increases the time to train a model and improves the performance up to a certain point.
...
The second part can be found here [Hyperband Series - Data Set Subsampling](/gallery/series/2022-12-02-hyperband-subsampling).
...
The next thing we do is defining the search space.
...
We load the Hyperband `r ref("TunerHyperband", "Hyperband tuner")` and set `eta = 2`.
...
Check out our [next post](/gallery/series/2022-12-02-hyperband-subsampling) on Hyperband which uses the size of the training data set as the fidelity parameter.
