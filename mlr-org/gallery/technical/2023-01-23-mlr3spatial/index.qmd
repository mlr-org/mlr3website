...
The package simplifies the work steps: Load the training data, create a spatial task, train a learner with it, and predict the final raster image (see @fig-flowchart).
...
The `r ref("as_task_classif_st()")` directly creates a spatial task from the point layer.
...
To predict a raster image, we use the `r ref("mlr3spatial::predict_spatial()")` function.
