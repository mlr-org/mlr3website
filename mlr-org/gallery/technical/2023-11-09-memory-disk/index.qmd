...
One cause for large sizes of parameter values is the use of source references, i.e. when installing packages with the `--with-keep.source` option.
Note that this option is enabled by default when installing packages with `renv`.
You can disble it by setting the following option before installing packages, e.g. by adding the following to your `.Rprofile`.
...
```{r}
pryr::object_size(serialie(lxy, NULL))
```
