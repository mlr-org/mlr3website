...
```{r index-009, eval=FALSE,code=readLines("mlr3_api/train_gl.R")}
```
...
```{r index-011, eval=FALSE,code=readLines("mlr3_api/fix_feature_types.R")}
```
...
```{r index-012, eval=FALSE,code=readLines("mlr3_api/predict_gl.R")}
```
...
```{r index-019, engine="bash",eval=FALSE}
...
```{r index-020, engine="bash",eval=FALSE}
...
```{r index-021, engine="bash",eval=FALSE}
RUN mkdir /data
COPY gl.rds /data
...
```{r index-022, engine="bash",eval=FALSE}
EXPOSE 1030
ENTRYPOINT ["R", "-e", \
...
```{r index-023, engine="bash",eval=FALSE,code=readLines("mlr3_api/Dockerfile")}
```

To build the image we open a terminal in the `mlr3_api` directory and run:

```{r index-024, engine="bash",eval=FALSE}
...
```{r index-025, engine="bash",eval=FALSE}
...
```{r index-026, engine="bash",eval=FALSE}
