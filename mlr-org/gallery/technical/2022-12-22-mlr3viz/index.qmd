...
The `autoplot()` function uses `r pkg_ref("ggplot2")` to draw a plot for almost any mlr3 object.
...
The `foritfy()` are handy to transform the mlr3 objects to `data.frames` that are readable by `r pkg_ref("ggplot2")`.
