...
When paired with doFuture, tidymodels exhibits significantly slower performance compared to the mlr3 package utilizing future (@tbl-resample-parallel-tidymodels-future-rpart and @tbl-resample-parallel-tidymodels-future-ranger).
We observed that tidymodels exports more data to the parallel workers, which notably exceeds that of mlr3.
This substantial difference in data export could plausibly account for the observed lag in runtime when using tidymodels on small tasks.
...
While `doParallel` provides a speed advantage over `mlr3`, its inability to consistently outpace the sequential approach in the context of `rpart` models points to the nuanced trade-offs that exist between different parallelization strategies and the inherent characteristics of various models.
