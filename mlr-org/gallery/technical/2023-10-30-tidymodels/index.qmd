...
Training an `rpart` model is slightly faster with `tidymodels` (@tbl-train-rpart).
...
For `ranger`, parallelization is faster than the sequential version (right @fig-resample-parallel).
