...
This process results in a *best feature (sub)set* and a *final trained model* using these best features, **for each combination of train set and learner** ($B \times M$).
...
In each RFE iteration, a 5-fold CV resampling of the given `task` takes place using the specified `learner`.
...
We define a `list()` with the following $M = 4$ classification `learners` (parameters are set at default values): 
...
Models that need minimum to no tuning (e.g. like Random Forests) are therefore ideal candidates for wrapper-based feature selection.
:::
...
Lastly, the performance of this final model will be assessed on the corresponding test set using the classification accuracy.
...
- The average rank importances for these 'best' features (`"importance"`) - this output column we get only because RFE optimization was used
- Inner optimization performance scores on the train sets (`"classif.ce_inner"`)
- Performance scores on the test sets (`"classif.acc"`)
...
By default the active measure is `"outer"`, i.e. the measure used to evaluate each learner's performance in the test sets.
...
:::{.callout-note}
In the following sections and analyses, we can use the inner optimization scores (i.e. `"classif.ce_inner"`) by executing `efs$set_active_measure("inner")`.
...
We observe that RF has better classification accuracy on the test sets of the $50$ subsamples, followed by XGBoost, then the SVM and last the tree model (as expected).
...
We observe that RF needed more features to achieve the average best performance (~20), followed by SVM (~15), then XGBoost (~12) and the tree model (~5) was the model using the least features (but with worst performance).
...
```{r efs-017}
#| fig-cap: "Number of features vs Accuracy with empirical pareto front"
#| warning: false
...
We can also draw an **estimated Pareto front curve** by fitting a linear model with the inversed of the number of selected features ($1/x$) of the empirical Pareto front as input, and the associated performance scores as output:

```{r efs-018}
#| fig-cap: "Number of features vs Accuracy with an estimated pareto front"
#| warning: false
...
Alternatively, the *Nogueira* measure can be used, which unlike the Jaccard Index, it's a chance-corrected similarity measure [@Nogueira2018]:
```{r efs-023}
#| warning: false
...
Using the Pareto method, we saw how we can identify a reasonable cutoff for the number of features.
Now we present some ways that allows us to create ranked list of features based on the ensemble feature selection process.
...
Below, we show the first 8 features, which correspond to the cutoff (`committee_size`) derived from the knee point of the estimated Pareto front.
The column `"score"` represents these counts:
```{r efs-024}
...
The primary objective is to compile these selections into a consensus ranked list of features (i.e a committee).
The column `"norm_score"` is the feature **selection frequency** or also known as **selection probabilities** [@Meinshausen2010].
...
Below, we use *satisfaction approval voting* (SAV), which results in a different feature ranking (e.g. `"V10"` feature is now included in the top 8 features):
```{r efs-026}
