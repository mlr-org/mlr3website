...
```{r index-009, message = FALSE}
hist(prediction$data$prob, breaks = (0:10)/10)
```

There are almost no cells with moderate probability estimates.

Then, assess if the model is calibrated with `calibration_plot()`. The calibration plot shows the relationship between the predicted probabilities and the true outcomes. The plot is divided into bins, and within each bin, the mean predicted probability and the mean observed outcome are calculated. The calibration plot can be smoothed by setting `smooth = TRUE`.

<details>
<summary>**Hint 1:**</summary>

`calibration_plot()` requires a `list` of learners even if the list contains only one argument.
...
```{r index-010}
calibration_plot(list(learner), cells_test)
```
...
learner_uncal = ...
resampling = ...
learner_cal = as_learner(po("calibrate", ...))
...
Check the documentation of `PipeOpCalibrate` with `??PipeOpCalibrate`.
...
learner_cal = as_learner(po("calibrate", learner = learner_uncal, calibrator = clb("platt"), resampling = resampling))
learner_cal$id <- "Platt Calibrated Learner"
```
...
calibration_plot(list(learner, learner_cal), cells_test, bins = 5)
```
...
learner_cal_iso = as_learner(po("calibrate", learner = learner_uncal, calibrator = clb("isotonic"), resampling = resampling))
...
calibration_plot(list(learner, learner_cal, learner_cal_iso), cells_test, bins = 5)
```
:::
...
`PipeOpCalibrate` can be treated as any other `PipeOp` object. Therefore, we can use them within more complex tuning and pipeline constructs, i.e. in cross-validation (CV), to assess calibration methods.
...
pipeline_cal = as_learner(po("calibrate", learner = ...,
                             resampling = ...,
                             calibrator = ...))
pipeline_cal$id <- "Beta Calibrated Learner"
pipeline_cal$...
calibration_plot(..., smooth = TRUE)
```
...
```{r index-017}
pipeline_cal = as_learner(po("calibrate", learner = lrn("classif.naive_bayes", predict_type = "prob"),
                             resampling = rsmp("cv", folds = 10),
                             calibrator = clb("beta")))
pipeline_cal$id <- "Beta Calibrated Learner"
pipeline_cal$train(cells_train)
calibration_plot(list(pipeline_cal), cells_test, bins = 5)
```
