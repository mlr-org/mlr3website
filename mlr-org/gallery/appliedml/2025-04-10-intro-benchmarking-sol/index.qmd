...
### Solution
$k$ around `r if (show.solution) as.numeric(gsub(".*?(\\d+).*", "\\1",  res[grepl("knn", learner_id)][which.max(classif.acc), "learner_id"])) else "22"` seems to perform best (in terms of accuracy). 
