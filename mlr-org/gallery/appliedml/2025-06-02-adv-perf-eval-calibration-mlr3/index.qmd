...
calibration_plot(list(learner), task_test, smooth = TRUE)
```
...
calibration_plot(list(learner, learner_cal), task_test, smooth = TRUE)
```
...
The ECE measure is constructed with `msr("classif.ece")` and passed to `$score()`.
...
po_filter = po(...)
pipeline = as_learner(... %>>% ...)
pipeline_cal = as_learner(po("calibrate", ...))
...
pipeline_cal = as_learner(po("calibrate", learner = pipeline,
                             resampling = rsmp("cv", folds = 10),
                             calibrator = clb("beta")))
pipeline_cal$id <- "Beta Calibrated Learner"
pipeline_cal$train(task_train)
calibration_plot(list(learner_cal, pipeline_cal), task_test, smooth = TRUE)
```
