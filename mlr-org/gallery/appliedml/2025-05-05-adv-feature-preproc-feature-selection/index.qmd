...
Create an `FSelectInstanceBatchSingleCrit` object using `fsi()`. The instance should use a 3-fold cross validation, classification accuracy as the `measure` and terminate after 20 evaluations. For simplification only consider the features `age`, `amount`, `credit_history` and `duration`.
...
```{r index-005, eval = params$showsolution}
...
Start the feature selection step by selecting `sequential` using the `FSelector` class via `fs()` and pass the `FSelectInstanceBatchSingleCrit` object to the `$optimize()` method of the initialized `FSelector` object.
...
```{r index-008, eval = params$showsolution}
...
```{r index-012, eval = params$showsolution}
...
```{r index-013, eval = params$showsolution}
...
```{r index-014, eval = params$showsolution}
...
```{r index-015, eval = params$showsolution}
...
```{r index-017, eval = params$showsolution}
task_gc = tsk("german_credit")
...
```{r index-019, eval = FALSE}
instance = fsi(...)
```

```{r index-020, eval = FALSE}
fselector = fs(...)
fselector$...(...)
```

```{r index-021, eval = FALSE}
...
```{r index-022, eval = params$showsolution, results='hide'}
instance = fsi(
  task = task_gc,
  learner = lrn_ranger,
  resampling = rsmp("cv", folds = 3),
...
```{r index-023, eval = params$showsolution, results='hide'}
fselector = fs("random_search")
fselector$optimize(instance)
```

```{r index-024, eval = params$showsolution}
features = unlist(lapply(instance$result$features, paste, collapse = ", "))
cbind(features,instance$result[,c("classif.tpr", "classif.tnr")])
```
Or with `data.table`:
```{r index-025, eval = params$showsolution, results='hide'}
...
```{r index-027, eval = params$showsolution}
afs = auto_fselector(
  fselector = fs("random_search"),
...
```{r index-028, eval = params$showsolution, results='hide'}
...
```{r index-029, eval = params$showsolution}
