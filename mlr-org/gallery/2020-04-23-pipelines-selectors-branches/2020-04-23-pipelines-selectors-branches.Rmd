...
```{r 2020-04-23-pipelines-selectors-branches-002, message=FALSE}
library(mlr3verse)
```
...
table_responsive(head(miss_ind$train(list(task_pima))[[1]]$data()))
```
...
```{r 2020-04-23-pipelines-selectors-branches-013, echo = FALSE}
#| column: page
DT::datatable(impute_missind$train(task_pima)[[1]]$data())
```
...
table_responsive(head(imputer_hist_2$train(task_pima)[[1]]$data()))
```
...
table_responsive(head(imputer_median_2$train(task_pima)[[1]]$data()))
```
...
```{r 2020-04-23-pipelines-selectors-branches-021, echo = FALSE}
#| column: page
DT::datatable(imputer_missind_2$train(task_pima)[[1]]$data())
```
...
```{r 2020-04-23-pipelines-selectors-branches-023, echo = FALSE}
#| column: page
DT::datatable(imputer_missind_3$train(task_pima)[[1]]$data())
```
...
DT::datatable(as.data.table(graph$param_set)[, c("id", "class", "lower", "upper", "nlevels"), with = FALSE])
```
...
#| column: page
DT::datatable(as.data.table(instance$archive, unnest = NULL, exclude_columns = c("x_domain", "uhash", "resample_result")))
```
...
```{r, include = FALSE}
# this is for the preview image in the gallery
#| include: false
utils::download.file(
  url = "https://mlr-org.com/gallery/2020-04-23-pipelines-selectors-branches/2020-04-23-pipelines-selectors-branches_files/figure-html5/2020-04-23-pipelines-selectors-branches-011-1.png",
