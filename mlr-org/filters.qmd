...
content = map_dtr(setdiff(mlr_filters$keys(), "ensemble"), function(key) {
  m = mlr_filters$get(key, .prototype = TRUE)
  list(key = key, label = m$label, task_types = list(m$task_types), packages = list(m$packages))
})
...
Use the $-\log_{10}()$-transformed $p$-values of a Kruskal-Wallis rank sum test (implemented in `r ref("kruskal.test()")`) for filtering features of the `r ref("mlr_tasks_diabetes", text = "Diabetes")` tasks.
