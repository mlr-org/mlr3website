...
learners = as.data.table(mlr_learners, objects = TRUE)
learner_man = set_names(map_chr(learners$object, function(x) if (is.null(x$man)) NA_character_ else x$man), learners$key)
...
learner_cell = function(value) {
  if (!length(value)) return(NULL)
  man = learner_man[[value]]
  if (is.null(man) || is.na(man)) return(value)
  mlr3website::ref(man, text = value, format = "htmltools")
}
...
# hyperparameter tuning on the diabetes data set
instance = tune(
