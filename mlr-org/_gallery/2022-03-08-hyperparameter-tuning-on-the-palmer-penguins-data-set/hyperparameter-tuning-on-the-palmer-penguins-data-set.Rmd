...
description: |
  In this post, we tune the hyperparameters of an rpart classification tree on the Palmer Penguins data set with only a few lines of code.
author:
  - name: Marc Becker
date: 2022-03-08
...
<div class="figure">
<p class="caption">Artwork by @horst_palmer_2022</p>
</div>
</div>
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-005, layout="l-body-outset", fig.cap="Flipper and bill length dimensions for Adelie, Chinstrap and Gentoo Penguins at Palmer Station [@horst_palmer_2022].", echo=FALSE}
...
We use the tuning space from the @bischl_hyperparameter_2021 article.
...
The column `logscale` indicate that the hyperparameters are tuned on the logarithmic scale.
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-010}
lts("classif.rpart.default")
```
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-011, eval=params$eval_all}
instance = tune(
  method = "grid_search",
  task = task,
  learner = learner,
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-012, echo=FALSE, eval=params$eval_all}
saveRDS(instance, "data/instance_1.rda")
```

```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-013, echo=FALSE}
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-014, eval=FALSE}
as.data.table(instance$archive)
```

```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-015, layout="l-body-outset", echo=FALSE}
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-016, eval=FALSE}
instance$result
```
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-018}
...
```{r hyperparameter-tuning-on-the-palmer-penguins-data-set-019, eval=FALSE}
learner = lrn("classif.rpart")
