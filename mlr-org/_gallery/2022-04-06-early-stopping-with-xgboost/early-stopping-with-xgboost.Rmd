...
```{r early-stopping-with-xgboost-009, echo=FALSE}
...
While tuning, the test sets of the `r ref("Resampling")` are used for early stopping.
When training the final model, early stopping must be used one last time.
We define a separate test set for this step.

```{r early-stopping-with-xgboost-013}
task = tsk("spam")
split = partition(task, ratio = 0.8)
task$set_row_roles(split$test, roles = "test")
```
