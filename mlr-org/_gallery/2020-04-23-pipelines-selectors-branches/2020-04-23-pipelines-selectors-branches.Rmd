...
responsive_table(head(miss_ind$train(list(task_pima))[[1]]$data()))
```
...
responsive_table(head(impute_missind$train(task_pima)[[1]]$data()))
```
...
responsive_table(head(imputer_hist_2$train(task_pima)[[1]]$data()))
```
...
responsive_table(head(imputer_median_2$train(task_pima)[[1]]$data()))
```
...
responsive_table(head(imputer_missind_2$train(task_pima)[[1]]$data()))
```
...
responsive_table(head(imputer_missind_3$train(task_pima)[[1]]$data()))
```
...
responsive_table(as.data.table(graph$param_set)[, c("id", "class", "lower", "upper", "nlevels"), with = FALSE])
```
...
responsive_table(as.data.table(instance$archive, unnest = NULL, exclude_columns = c("x_domain", "uhash", "resample_result")))
```
