...
responsive_table(as.data.table(instance$archive, exclude_columns = c("uhash",  "resample_result")))
```
...
responsive_table(extract_inner_tuning_results(rr)[, c("iteration", "branch.selection", "classif.ce", "task_id", "learner_id", "resampling_id"), with = FALSE])
```
...
responsive_table(extract_inner_tuning_results(rr))
```
