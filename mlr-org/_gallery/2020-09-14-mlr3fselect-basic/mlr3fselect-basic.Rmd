...
```{r mlr3fselect-basic-014, echo = FALSE}
responsive_table(as.data.table(instance$archive, exclude_columns = c("runtime_learners", "resample_result", "uhash")))
```
...
```{r mlr3fselect-basic-020, eval = FALSE}
...
responsive_table(as.data.table(instance$archive, exclude_columns = c("runtime_learners", "timestamp", "batch_nr", "resample_result", "uhash")))
```
...
```{r mlr3fselect-basic-023, eval = params$eval_all}
resampling_outer = rsmp("cv", folds = 3)

rr = resample(task, at, resampling_outer, store_models = TRUE)
```

```{r mlr3fselect-basic-024, echo = FALSE, eval = params$eval_all}
saveRDS(rr, "data/rr.rda")
```

```{r mlr3fselect-basic-025, echo = FALSE}
...
```{r mlr3fselect-basic-027, eval = FALSE}
extract_inner_fselect_results(rr)
```

```{r mlr3fselect-basic-028, echo = FALSE, layout="l-body-outset"}
responsive_table(extract_inner_fselect_results(rr))
```
...
```{r mlr3fselect-basic-030, echo = FALSE}
responsive_table(rr$score()[, c("iteration", "task_id", "learner_id", "resampling_id", "classif.ce"), with = FALSE])
```
...
```{r mlr3fselect-basic-032, echo = FALSE, layout="l-page"}
responsive_table(extract_inner_fselect_archives(rr))
```
...
```{r mlr3fselect-basic-034, eval = FALSE}
rr = fselect_nested(
  method = "random_search",
  task = tsk("iris"),
  learner = lrn("classif.log_reg"),
