  ...
  div(class = "grid-container-package",
    pmap(content, function(category, name, status, url, text, on_cran, ...) {
        div(class = "grid-item-package",
          h3(
            a(href = url, name),
            tags$i(class = sprintf("bi-circle-fill status status-%s", status))
          ),
          p(paste0(text, ".")),
          div(id = "bottom",
            if (!startsWith(url, "https://github.com")) a(href = url, tags$i(class = "bi bi-house")),
            a(href = sprintf("https://github.com/mlr-org/%s", name), tags$i(class = "bi bi-github")),
            if (on_cran) a(href = sprintf("https://cran.r-project.org/package=%s", name), tags$i(class = "fab fa-r-project"))
          )
        )
    })
  )
