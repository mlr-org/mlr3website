...
* [Do I have to set the environment variable `OMP_NUM_THREADS`?](#omp)
...
Prior to `mlr3` version 0.17.1, the `r ref_pkg("data.table")` package used half of the available cores of the system on each worker.
This overloaded the CPUs when using `r ref("resample()")` and `r ref("benchmark()")` with `r ref_pkg("future")` and led to slower execution times.
Setting the environment variable `OMP_NUM_THREADS` is no longer necessary.
