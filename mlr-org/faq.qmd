...
* `ResampleResult` and `BenchmarkResult` objects stored with mlr3 <= 1.2.0 cannot be loaded with mlr3 >= 1.3.0 because `presence` and `allow_token` were added to the `$check()` method of the `ParamSet` class. Other objects with `ParamSet` objects could also be affected.
